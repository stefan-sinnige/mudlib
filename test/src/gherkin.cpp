...
void
mud::test::_feature_factory::register_feature(const std::string& id,
                                              creator_func func)
{
    _map[id] = func;
}
...
mud::test::_feature_factory::run(const std::string& test /* = std::string() */)
{
    ...
    for (auto entry : _map) {
        ...
        std::pair<size_t, size_t> feature_result =
            feature->run(filter_scenario);
    ...
    std::cout << std::endl
              << "Total: " << result.first << "  "
              << "Pass: " << result.second << "  "
              << "Fail: " << result.first - result.second << std::endl;
    return result;
}
