...
struct WSAInitialiser
{
    WSAInitialiser()
    {
        ...
        if (WSAStartup(version, &wsa) != 0) {
            std::cerr << "Error initialising WSA [" << WSAGetLastError() << "]"
                      << std::endl;
            exit(1);
        }
    }
...
int g_domains[] = {
    AF_UNIX,   AF_UNIX,          /** LOCAL */
    AF_INET,   AF_INET6,     -1, /** AX25 */
    AF_IPX,    AF_APPLETALK, -1, /** X25 */
    AF_DECnet, -1,               /** KEY */
    -1,                          /** NETLINK */
    -1,                          /** PACKET */
    -1,                          /** RDS */
...
int g_protocols[] = { IPPROTO_IP, /** INTRINSIC */
                      IPPROTO_IP,
                      IPPROTO_HOPOPTS,
                      IPPROTO_ICMP,
                      IPPROTO_IGMP,
