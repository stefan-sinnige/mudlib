...
streambuf::streambuf(
    ...
    : mud::io::basic_streambuf(
              ...
              bufsize,
              putbacksize),
      _socket(socket), _source_endpoint(source_endpoint),
...
streambuf::read(void* buf, size_t count)
{
    ...
    if (nread >= 0)
    {
        ...
        udp::endpoint destination(addr.sin_addr.s_addr, ntohs(addr.sin_port));
    ...
    : _socket(socket), _handle(handle), _istr(nullptr), _ostr(nullptr)
{
     ...
     * stream objects. */
    _read_buffer  = std::unique_ptr<_udp::streambuf>(
                    new _udp::streambuf(_socket, _handle, _source_endpoint,
                            _destination_endpoint, 4096, 16));
    _write_buffer = std::unique_ptr<_udp::streambuf>(
                    new _udp::streambuf(_socket, _handle, _source_endpoint,
                            _destination_endpoint, 4096, 16));
